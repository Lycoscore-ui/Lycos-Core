<?php
/*
Plugin Name: Cipher AI Chat API Endpoint
Description: Intercepts /api/cipher requests and serves Cipher AI Chat responses via Gemini Interactions API.
Version: 1.0
*/

lycos_cipher_api_router();

function lycos_cipher_api_router() {
    $requestUri = $_SERVER['REQUEST_URI'] ?? '';
    
    // Check if request is targeting any cipher route or endpoint
    if (strpos($requestUri, 'cipher') === false) {
        return;
    }
    ...
    // 1. Parse GEMINI_API_KEY from v2-development/cms/.env or environment
    $apiKey = getenv('GEMINI_API_KEY');

    if (empty($apiKey)) {
        $envPaths = [
            ABSPATH . '../v2-development/cms/.env',
            ABSPATH . 'v2-development/cms/.env',
            dirname(ABSPATH) . '/v2-development/cms/.env',
            'C:/Users/potta/Local Sites/lycos-core/v2-development/cms/.env'
        ];
    ...
    $kbPath = ABSPATH . 'lycos_core_knowledge.txt';
    if (!file_exists($kbPath)) {
        $kbPath = dirname(ABSPATH) . '/lycos_core_knowledge.txt';
    }
    ...
    foreach ($modelsToTry as $modelName) {
        ...
        if ($httpCode >= 200 && $httpCode < 300 && !empty($response)) {
            ...
            if (!isset($resData['error'])) {
                ...
                // Extract from steps array (Interactions API response structure)
